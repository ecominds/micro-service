...
Note: If we do not have any option to manage the the volume, we can create a docker container and then after copying the config file to the volume, delete the container (if not in use).
...
* and map the volume to the container
...
```
Note: The above command will eventually copy the `app_config.yml` file to the volume `local_dev_nfs`. Now, even if we delete the container, the configuration file will be there in `local_dev_nfs` volume.
